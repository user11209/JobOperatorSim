...
def init(env_file, workload_file, sched_policy):
    ...
    err = gen_error(GPU_num=gpu_num, High_err_num=env_para["High_error_card"],
                    ...
                    job_list=scheduler.init_workflow(workloads),scale = env_para["Scale"])
    return workloads, err, scheduler, gpu_num, env_para
        ...
        if i > 0:
            ...
            for j in range(len_key):
                if dic_key[j] in ["submit_time", "running_time", "GPU_num", "restart", "real_sub", "real_running",
                                  "preempt_times", "err_times", "place_times","epoch_time","total_time"]:
...
def gpu_limit_time(Workload, jobs, gpu_rest_num):
    '''
...
#    return l
def gen_int_point(up_num, num):
    l = list(range(up_num+1))
    for i in range(num):
        switch_num = random.randint(0,up_num-i)
        temp = l[switch_num]
        l[switch_num] = l[up_num-i]
        l[up_num-i] = temp
    l = l[up_num-num+1:up_num+1]
    l.sort()
    return l
...
def gen_int_point_expt(up_num, num,expt):
    len_expt = len(expt)
    l = list(range(up_num+1))
    l = list(set(l) - set(expt))
    for i in range(num):
        switch_num = random.randint(0,up_num-i-len_expt)
        temp = l[switch_num]
        l[switch_num] = l[up_num-i-len_expt]
        l[up_num-i-len_expt] = temp
    l = l[up_num-num+1-len_expt:up_num+1-len_expt]
    l.sort()
    return l
...
def gen_error(GPU_num, High_err_num, High_err_rate, Low_err_num, Low_err_rate, Max_err_num, Workload, job_list, recover_time = 10, scale = 60):
...
def gen_metric(finish_dict,workload,out_file,tag):
    total_wait_time = 0
    total_execute_time = 0
    success_time = 0
    success_count = 0
    total_first_attempt_time = 0
    count = 0
    for job_id in workload.keys():
        total_wait_time = total_wait_time + finish_dict[job_id][-1][1]#- workload[job_id]["submit_time"]
        total_execute_time += finish_dict[job_id][-1][-1]
        temp_value = finish_dict[job_id][0][1] - finish_dict[job_id][0][0]
        if temp_value >= workload[job_id]["epoch_time"]:
            success_time += 1
        else:
            total_first_attempt_time += temp_value
        count = count + 1
    average_wait_time = total_wait_time / count    
    print("average_wait_time: ",average_wait_time)
    print("average_execute_time: ",total_execute_time / (count-success_count))
    print("success_rate: ",success_time/count)
    print("average_first_attempt_time: ",total_first_attempt_time/success_time)
    fd = open(out_file,'a',newline='')
    fd.write(tag+": "+str(average_wait_time)+'\n')
    fd.close()
